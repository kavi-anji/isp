...
def getAttributess(url):
    ...
    d={'Having_@_symbol':pd.Series(having_at_symbol),'Having_IP':pd.Series(having_ip),'Prefix_suffix_separation':pd.Series(prefix_suffix_separation),
     'Redirection_//_symbol':pd.Series(redirection_symbol),'Sub_domains':pd.Series(sub_domains),'URL_Length':pd.Series(len_url),'age_domain':pd.Series(age_domain),
    'dns_record':pd.Series(dns_record),'domain_registration_length':pd.Series(domain_registration_length),'http_tokens':pd.Series(http_tokens),'statistical_report':pd.Series(statistical_report),
